...
// Model provider (configurable: ollama, foundry-local)
var modelProvider = builder.Configuration.GetValue<string>("Model:Provider") ?? "ollama";
switch (modelProvider.ToLowerInvariant())
{
    case "foundry-local":
        builder.Services.AddFoundryLocal(o =>
        {
            o.Model = builder.Configuration.GetValue<string>("Model:Model") ?? "phi-4";
        });
        break;
    default:
